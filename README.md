...
This repository is configured for Railway using the included `Dockerfile`. Railway should detect the Dockerfile automatically; the explicit start command is `python main.py`.

Set `TELEGRAM_BOT_TOKEN` in Railway Variables. `HEADLESS=True`, `CHROME_BINARY=/usr/bin/chromium`, and `CHROMEDRIVER_PATH=/usr/bin/chromedriver` are supplied by the container defaults. The optional variables `PROXY`, `WEBDRIVER_TIMEOUT`, `IMPLICIT_WAIT`, `PAGE_LOAD_TIMEOUT`, and `LOG_LEVEL` may be overridden when needed.

The bot uses `python-telegram-bot==21.10`. Do not install the unrelated PyPI distribution named `telegram`. The application deletes any existing Telegram webhook asynchronously during PTB startup, then begins polling with stale updates dropped.

The included `runtime.txt` documents Python 3.12.10 for non-Docker Railway builders. When the Dockerfile is selected, its `python:3.12-slim` base image is authoritative.
